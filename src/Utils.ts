...
export function resolveReportFile(playwrightConfigFile?: string) {
  const outputPath = playwrightConfigFile
...
export function getReportFromFile(
  file: string,
  createExampleFileIfMissing = false,
) {
  if (!fs.existsSync(file)) {
    if (createExampleFileIfMissing) {
      ...
      throw new Error(
        `Visual regression report file does not exist. An example has been created in ${reportFileName}.`,
      );
    } else {
  ...
  if (
    !rawReport.config ||
    !rawReport.config.referenceUrl ||
    !rawReport.config.subjectUrl
  ) {
    throw new Error(
      "Visual regression report file must at least specify config.referenceUrl and config.subjectUrl.",
    );
  }
  ...
  rawReport.config.sitemapUrl ??=
    rawReport.config.referenceUrl + "/sitemap.xml";
  rawReport.config.threshold ??= 0.2;
  rawReport.config.increaseWaitForRetry ??= true;
  rawReport.config.cachePath ??= path.join(
    path.dirname(file),
